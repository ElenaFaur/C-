...
int main()
{
   ...
   /* for(int i:v3)
    ...
    auto position=find(begin(source),end(source),1); //cout<<*position;
   ...
   /* vector<Resource> vr(2);
    ...
    unique_copy(begin(v8),end(v8),begin(v7));
     ...
     reverse(begin(sentence),end(sentence)); //cout<<sentence;
    ...
    string orig="                 ";
    reverse_copy(begin(sentence),end(sentence),begin(orig)); cout<<orig<<" ";
}